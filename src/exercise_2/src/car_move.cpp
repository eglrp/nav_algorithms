...
int main(int argc, char** argv) {
  ...
  given_waypoints.push_back(Point(4.0, 2.0));
  given_waypoints.push_back(Point(3.0, 3.0));
...
vector<Speed> trajectoryPlan(const vector<Point>& waypoints, const Pose& cur_pose, const Speed& cur_speed, double dt, double command_length)
{
  ...
  for (int i = 0; i < path_points.size() - 1; i++)
  {
    ...
    //double roal = sprt( pow(delta_x,2) + pow(delta_y,2));
    ...
    if (delta_angle >= 1.0*PI ) delta_angle -= 2.0*PI;
    if (delta_angle < -1.0*PI ) delta_angle += 2.0*PI;
    ...
    for (int j = 0; j < command_per_segment - 1; j++)
    {
      ...
      temp_command.v = 3 * segment_length;
      if(temp_command.v >= 0.8) temp_command.v = 0.8;

      temp_command.w = 8*delta_angle + 1.5*segment_angle;
      if(temp_command.w >= 1.0*PI /3.0 ) temp_command.w = 1.0*PI /3.0;
      if(temp_command.w <= -1.0*PI /3.0 ) temp_command.w = -1.0*PI /3.0;

      command_remain= command_remain-1;
      plan_result.push_back(temp_command);

      robot_orientation += temp_command.w * dt; //segment_angle;
      if (robot_orientation >= 1.0*PI ) robot_orientation -= 2.0*PI;
      if (robot_orientation < -1.0*PI ) robot_orientation += 2.0*PI;
